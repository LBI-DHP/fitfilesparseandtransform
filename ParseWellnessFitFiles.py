    ...
    for message in fitfile.get_messages(data_frame_name):
        ...
        for data in message:
            if(data.name == "timestamp"):
                ...
                local_datetime = unix_datetime + dt.timedelta(seconds=time_offset)
                ...
                unix_timestamp_garmin_epoch  = int(dt.datetime.timestamp(unix_datetime)) - 631065600
                timestamp_16 = data.value
                calculated_timestamp_garmin_epoch  =  unix_timestamp_garmin_epoch + ((timestamp_16 -  unix_timestamp_garmin_epoch) & 0xffff)
                local_calculated_datetime = dt.datetime.utcfromtimestamp(calculated_timestamp_garmin_epoch + 631065600 + time_offset)
...
for filename in filenames:
    ...
    if(file_type == "monitoring_b"): 
        ...
        dataframe = appendToDataframe("monitoring", fitfile, time_offset, dataframe)
