...
for filename in filenames:
    ...
    if(file_type == "activity"):
        ...
        dataframe = appendToDataframe(
            "record", fitfile, time_offset, dataframe)

        if(resample):
            dataframe.index = pandas.to_datetime(
                dataframe.local_time, format='%Y-%m-%D %H:%M:%S')
        ...
        try:
            dataframe.to_excel(OUTPUT_FOLDER + os.path.splitext(filename)
                               [0]+"_fitfilev"+str(sw_version)+".xlsx")
